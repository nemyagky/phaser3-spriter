...
export class SpriterXml extends SpriterFile {
  ...
  public getCharMapEntry(element: Element, charMap: CharMap, spriter: Spriter): void {
    var sourceName = spriter.getFolderById(this.parseInt(element, "folder")).getFileById(this.parseInt(element, "file")).name;

    var target: File = null;
    if (element.hasAttribute("target_folder") && element.hasAttribute("target_file")) {
      target = spriter.getFolderById(this.parseInt(element, "target_folder")).getFileById(this.parseInt(element, "target_file"));
    }
